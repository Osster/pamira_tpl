...
<div class="cart_item_list" id="id-shelve-list" style="display:none;">
    <div class="cart_item_list_top_container">
        ...
        <ul>
            <li ontouchstart="ShowBasketItems(1);"><a
                        href="javascript:void(0)"><?= GetMessage("SALE_PRD_IN_BASKET_ACT") ?> <span
                            class="cart-item-title">(<?= count($arResult["ITEMS"]["AnDelCanBuy"]) ?>)</span></a></li>
            <li class="current"><a href="javascript:void(0)"><?= GetMessage("SALE_PRD_IN_BASKET_SHELVE") ?> <span
                            class="delay-item-title">(<?= count($arResult["ITEMS"]["DelDelCanBuy"]) ?>)</span></a></li>
        </ul>
        <div class="clb"></div>
    </div>
    <ul id="id-shelve-ul">
			...
			<?endif;*/
                    ...
                    if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
                        <div class="cart_item_count">
                            ...
                            <a href="javascript:void(0)" class="count_minus"
                               ontouchstart="if (BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value > 1) BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value--;"
                            ...
                            <input maxlength="18" min="1" type="number" class="quantity_input"
                                   ...
                                   size="3" id="QUANTITY_<?= $arBasketItems["ID"] ?>" readonly>
                            <a href="javascript:void(0)" class="count_plus"
                               ontouchstart="BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value++;"
                               style="display:none"><span></span></a>
                        </div>
                    <? endif; ?>
                    ...
                    if (in_array("DELETE", $arParams["COLUMNS_LIST"])):?>
                        <a class="cart_item_remove"
                           ...
                           onclick="return DeleteFromCart(this);" title="<?= GetMessage("SALE_DELETE_PRD") ?>"></a>
                    <? endif; ?>
                    <?
                    if (in_array("DELAY", $arParams["COLUMNS_LIST"])):?>
                        <a class="cart_item_delayed"
                           href="<?= str_replace("#ID#", $arBasketItems["ID"], $arUrlTempl["add"]) ?>"
                           onclick="return Add2Order(this)"></a>
                    <? endif; ?>
                </li>
                <?
            }
