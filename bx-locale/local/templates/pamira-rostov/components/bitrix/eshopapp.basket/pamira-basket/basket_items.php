...
<? if (count($arResult["ITEMS"]["AnDelCanBuy"]) > 0): ?>
    ...
    foreach ($arResult["ITEMS"]["AnDelCanBuy"] as $arBasketItems) {
        ?>
        <div class="basket__product" id="basketItemID_<?= $arBasketItems["ID"] ?>" <?
			...
			<?endif;*/
                    ...
                    if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
                        <div class="basket-calc">
                            ...
                            <a href="javascript:void(0)" class="basket-calc__minus"
                               ontouchstart="if (BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value > 1) BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value--;"><span></span></a>
                            <input style="width: 50px;" maxlength="18" min="1" type="number" class="basket-calc__number"
                                   name="QUANTITY_<?= $arBasketItems["ID"] ?>" value="<?= $arBasketItems["QUANTITY"] ?>"
                                   size="3" id="QUANTITY_<?= $arBasketItems["ID"] ?>">
                            <a href="javascript:void(0)" class="basket-calc__plus"
                               ontouchstart="BX('QUANTITY_<?= $arBasketItems["ID"] ?>').value++;"><span></span></a>
                        </div>
                    <? endif; ?>
...
<div class="basket__result" id="cart_item_bottom"
...
<div class="basket__buttons">
    <a href="javascript:void(0)" class="btn btn-hover_white" ontouchstart="BX.toggleClass(this, 'active');"
       ontouchend="BX.toggleClass(this, 'active');" onclick="
    ...
    <a id="basketOrderButton2" class="btn btn-hover_red" ontouchstart="BX.toggleClass(this, 'active');"
       ontouchend="BX.toggleClass(this, 'active');"
