...
<?foreach($arResult["MENU_STRUCTURE"] as $itemID => $arColumns):?>     <!-- first level-->
    ...
    <li
            class="nav-item nav-menu__list_item bx-nav-1-lvl bx-nav-list-<?= ($existPictureDescColomn) ? count($arColumns) + 1 : count($arColumns) ?>-col <? if ($arResult["ALL_ITEMS"][$itemID]["SELECTED"]): ?>active<? endif ?><? if (is_array($arColumns) && count($arColumns) > 0): ?> bx-nav-parent<? endif ?>"
