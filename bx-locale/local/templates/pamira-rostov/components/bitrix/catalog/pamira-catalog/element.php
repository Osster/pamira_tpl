    ...
    <section class="main__promo">
        <div id="card-collapse" class="container">
            <div class="row">
                ...
                <?
                $ar_group_ids = [];
                $elem_group_res = CIBlockElement::GetElementGroups(
                    $elementId,
                    false,
                    ["ID"]
                );
                while ($ar_group = $elem_group_res->Fetch()) {
                    $ar_group_ids[] = $ar_group["ID"];
                }

                $group_res = CIBlockSection::GetList([],
                    [
                        "ID" => $ar_group_ids,
                        "IBLOCK_ID" => $arParams["IBLOCK_ID"],
                    ],
                    false, ["IBLOCK_ID", "UF_ADVANTAGES_DIR"]);

                $rel_IBLOCK_ID = 0;
                $ar_related_group_ids = [];
                while ($ar_group = $group_res->GetNext()) {
                    if (!empty($ar_group["UF_ADVANTAGES_DIR"])) {
                        $rel_IBLOCK_ID = $ar_group["IBLOCK_ID"];
                ...
                $advantagesFilter = [];
                if (!empty($ar_related_group_ids)) {
                    $filter["SECTION_ID"] = $ar_related_group_ids;

                    $er = CIBlockElement::GetList(Array("SORT" => "ASC"), $filter, false, false,
                        ["ID",
                            "NAME",
                            "PREVIEW_TEXT",
                            "PREVIEW_PICTURE",
