            ...
            case 'ADD_TO_COMPARE_LIST':
                ...
                if (!isset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$productID])) {
                    ...
                    if ($found) {
                        ...
                        $picId = $arElement['PREVIEW_PICTURE'] ? $arElement['PREVIEW_PICTURE'] : $arElement['DETAIL_PICTURE'];

                        $arElement['PICTURE'] = false;
                        if (intval($picId) > 0) {
//                            $picRes = CFile::getById($picId);
//                            $arElement['PICTURE'] = $picRes->GetNext();
                            $arElement['PICTURE'] = CFile::GetPath($picId);
                        }

                        $arElement["PRICE"] = '';
                        if ($arPrice = CPrice::GetBasePrice($arElement["ID"])) {
                            $arElement["PRICE"] = CurrencyFormat($arPrice["PRICE"], $arPrice["CURRENCY"]);
                        }
