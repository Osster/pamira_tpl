<div class="container">
    <ul class="row main_sections-nav_level-0 nav" id="myTab" role="tablist">

        <li class=" col-6 col-lg-3 nav-item">
            <a class="main_sections-nav_card active" id="santeh-tab" data-toggle="tab" role="tab"
               href="#santeh" aria-controls="santeh" aria-selected="true">
                <svg width="41" height="50" data-toggle="tooltip" data-placement="left"
                     title="Сантехника">
                    <use xlink:href="#santehnika"></use>
                </svg>
                <span class="label">Сантехника</span>
            </a>
        </li>
        <li class="col-6 col-lg-3 nav-item">
            <a class="main_sections-nav_card" id="tehniksDlyaKuhni-tab" data-toggle="tab" role="tab"
               href="#tehniksDlyaKuhni" aria-controls="tehniksDlyaKuhni" aria-selected="false">
                <svg width="41" height="50" data-toggle="tooltip" data-placement="left"
                     title="Техника для кухни">
                    <use xlink:href="#tehnika_dla_kuhni"></use>
                </svg>
                <span class="label">Техника для кухни</span>
            </a>
        </li>
        <li class="col-6 col-lg-3">
            <a class="main_sections-nav_card" href="/tehnika/mbt-i-posuda/">
                <svg width="41" height="50" data-toggle="tooltip" data-placement="left"
                     title="МБТ и посуда">
                    <use xlink:href="#mbt_i_posuda"></use>
                </svg>
                <span class="label">МБТ и посуда</span>
            </a>
        </li>
        <li class="col-6 col-lg-3 nav-item">
            <a class="main_sections-nav_card" id="uhodZaBelem-tab" data-toggle="tab" role="tab"
               href="#uhodZaBelem" aria-controls="uhodZaBelem" aria-selected="false">
                <svg width="41" height="50" data-toggle="tooltip" data-placement="left"
                     title="Уход за бельем">
                    <use xlink:href="#uhod_za_belem"></use>
                </svg>
                <span class="label">Уход за бельем</span>
            </a>
        </li>


    </ul>


    <div class="row tab-content" id="myTabContent">

        <div class="col-12 main_sections-nav_level-1 tab-pane fade show active" id="santeh" role="tabpanel"
             aria-labelledby="santeh-tab">

            <div class="row">

                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/kukhonnye-moyki/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Кухонные мойки">
                            <use xlink:href="#kuhonnye_moiki"></use>
                        </svg>

                        <span class="label">Кухонные мойки</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/izmelchiteli/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Измельчители">
                            <use xlink:href="#izmelchiteli"></use>
                        </svg>
                        <span class="label">Измельчители</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/sortery/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Сортеры">
                            <use xlink:href="#sortery"></use>
                        </svg>
                        <span class="label">Сортеры</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/kukhonnye-moyki/aksessuary-dlya-moek/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Аксессуары">
                            <use xlink:href="#aksessuary"></use>
                        </svg>
                        <span class="label">Аксессуары</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/dozatory-dlya-myla/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Дозаторы">
                            <use xlink:href="#dozatory"></use>
                        </svg>
                        <span class="label">Дозаторы</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/santekhnika/smesiteli/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Смесители">
                            <use xlink:href="#smesiteli"></use>
                        </svg>
                        <span class="label">Смесители</span>
                    </a>
                </div>

            </div>

        </div>


        <div class="col-12 main_sections-nav_level-1 tab-pane fade" id="tehniksDlyaKuhni" role="tabpanel"
             aria-labelledby="tehniksDlyaKuhni-tab">

            <div class="row">

                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/dukhovye-shkafy/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Духовые шкафы">
                            <use xlink:href="#duhovie_shkafi"></use>
                        </svg>
                        <span class="label">Духовые шкафы</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/kholodilnoe-oborudovanie/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Смесители">
                            <use xlink:href="#holodilniki"></use>
                        </svg>
                        <span class="label">Холодильное оборудование</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/varochnye-poverkhnosti/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Смесители">
                            <use xlink:href="#varochnie_poverhnosti"></use>
                        </svg>
                        <span class="label">Варочные поверхности</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/kompaktnye-pribory/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Компактные приборы">
                            <use xlink:href="#kompaktnie_pribori"></use>
                        </svg>
                        <span class="label">Компактные приборы</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/kukhonnye-bloki/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Кухонные блоки">
                            <use xlink:href="#kuhonnye_bloki"></use>
                        </svg>
                        <span class="label">Кухонные блоки</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/vytyazhki/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Вытяжки">
                            <use xlink:href="#vytazhki"></use>
                        </svg>
                        <span class="label">Вытяжки</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/mikrovolnovye-pechi//">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Микроволновые печи">
                            <use xlink:href="#mikrovolnovie_pechi"></use>
                        </svg>
                        <span class="label">Микроволновые печи</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/tekhnika-dlya-kukhni/posudomoechnye-mashiny/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Посудомоечные машины">
                            <use xlink:href="#posudomoechnie_mashyny"></use>
                        </svg>
                        <span class="label">Посудомоечные машины</span>
                    </a>
                </div>

            </div>

        </div>


        <div class="col-12 main_sections-nav_level-1 tab-pane fade" id="uhodZaBelem" role="tabpanel"
             aria-labelledby="uhodZaBelem-tab">

            <div class="row">

                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/ukhod-za-belem/stiralnye-mashiny/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Стриральные машины">
                            <use xlink:href="#stiralnye_mashyny"></use>
                        </svg>
                        <span class="label">Стриральные машины</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/ukhod-za-belem/sushilnye-mashiny/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Сушильные машины">
                            <use xlink:href="#sushylnye_mashyny"></use>
                        </svg>
                        <span class="label">Сушильные машины</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/ukhod-za-belem/sushilnye-shkafy/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Сушильные шкафы">
                            <use xlink:href="#sushylnye_shkafy"></use>
                        </svg>
                        <span class="label">Сушильные шкафы</span>
                    </a>
                </div>
                <div class="col-4 col-lg-3">
                    <a class="main_sections-nav_card main_sections-nav_card_content"
                       href="/tehnika/ukhod-za-belem/aksessuary-dlya-tekhniki-po-ukhodu-za-belyem/">
                        <svg width="50" height="52" data-toggle="tooltip" data-placement="left"
                             title="Аксессуары">
                            <use xlink:href="#aksessuari_uhod_za_beliom"></use>
                        </svg>
                        <span class="label">Аксессуары</span>
                    </a>
                </div>

            </div>

        </div>


    </div>

</div>